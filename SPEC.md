...
| Stage 4 model | `qwen3-vl:4b` via a local Ollama server (`localhost:11434`), not Claude Haiku — no paid API available, so switched to local. Local also means no API key, no per-call cost, and no rate limit to manage. Chosen over `gemma3:4b` per a standalone smoke test: both returned valid, schema-compliant JSON on every call via Ollama's structured-output `format` support, but `gemma3:4b` incorrectly verdicted "yes" on a clearly non-matching image (and misdescribed its content), while `qwen3-vl:4b` correctly discriminated both a matching and a non-matching case — JSON reliability was tied, so this correctness gap decided it, outweighing `gemma3:4b`'s edge on VRAM (3.8GB vs 4.3GB, both comfortably under the 6GB GPU's limit) and latency (~4s vs ~6s warm-call). Single provider, no abstraction layer for v1. |
| Ollama availability | Stage 4 fails fast with a clear error naming the missing server (e.g. "Ollama server not reachable at localhost:11434 -- is it running?") if the Ollama server isn't running when the stage starts, rather than surfacing a raw connection exception. Checked once up front, not per-frame. |
